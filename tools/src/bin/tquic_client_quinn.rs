...
use tquic::Shutdown::Read;
use tquic::TlsConfig;
use tquic::TransportHandler;
use tquic_tools::ApplicationProto;
use tquic_tools::QuicSocket;
use tquic_tools::Result;
use crate::OptimizationAlg::{GradientDecent, Newton};
...
struct TrafficData {
    timestamp: f64,
    stream_count: u64,
    throughtput: f64, // in Mbps
    filename: String,
}

impl TrafficData {
    fn new(timestamp: f64, stream_count: u64, throughtput: f64, filename: String) -> Self {
        TrafficData {
            timestamp,
            stream_count,
            throughtput,
...
fn save_data_to_csv(data: TrafficData) -> io::Result<()> {
    ...
    // 如果文件是新的，写入表头
    if metadata.len() == 0 {
        wtr.write_record(&["timestamp", "stream_count", "throughtput"])?;
    }
    wtr.write_record(&[data.timestamp.to_string(), data.stream_count.to_string(), data.throughtput.to_string()])?;
...
impl OptimizationParas {
    ...
    pub fn update(&mut self, conn: &mut Connection, last_stats: &mut ConnectionStats, last_time: &mut Instant, start_time: &mut Instant, k: f64, b: f64, save_flag: bool, filename: String) -> Result<()> {
        // 更新 rtt_min 和 rtt_now
        // 先存储引用到局部变量中
        let path_stats = conn.get_active_path_mut().unwrap().stats();
        let min_rtt = path_stats.min_rtt.clone();
        let srtt = path_stats.srtt.clone();
        ...
        // 获取当前连接的统计数据和当前时间
        let now_stats = conn.stats();
        let now_time = Instant::now();

        // 计算 t 和 L 和 u
        self.t = ((now_stats.recv_bytes as f64 - last_stats.recv_bytes as f64)
            / (now_time.duration_since(*last_time).as_secs_f64() * self.n as f64));
        self.l = (now_stats.lost_count as f64 - last_stats.lost_count as f64)
            / (now_stats.sent_count as f64 - last_stats.sent_count as f64);
        self.calculate_u(k, b).unwrap();
        println!("BD = {}", self.t * self.n as f64/131072.0);
        if save_flag {
            let data = TrafficData::new(now_time.duration_since(*start_time).as_secs_f64(), self.n as u64, self.t * self.n as f64 / 131072.0, filename);
            save_data_to_csv(data).unwrap();
        }


        // 更新 last_stats
        last_stats.lost_count = now_stats.lost_count;
        last_stats.lost_bytes = now_stats.lost_bytes;
        last_stats.recv_count = now_stats.recv_count;
        last_stats.sent_count = now_stats.sent_count;
        last_stats.recv_bytes = now_stats.recv_bytes;
        last_stats.sent_bytes = now_stats.sent_bytes;
    ...
    // 定义计算函数
    pub fn calculate_u(&mut self, k: f64, b: f64) -> Result<()> {
        self.u = ((self.n as f64 * self.t).powf(0.999)  - self.n as f64 * self.t * self.l * b)* self.t.powf(0.7);
        Ok(())
    }

    // 定义目标函数的导数
    pub fn calculate_u_derivative(&mut self, last_paras: &OptimizationParas, k: f64, b: f64) -> Result<()> {
        self.gradient = (self.u - last_paras.u) / 2.0;
        Ok(())
    }

    // 计算一阶导数
    pub fn first_derivative(&self, k: f64, b: f64) -> f64 {
        let part1 = (self.rtt_min as f64 * self.n as f64 * self.t) / (self.rtt_now as f64 * k.powf(self.n as f64));
        let part2 = 1.0 - self.n as f64 * k.ln();
        let part3 = part1 * part2;
        let part4 = self.t * self.l * b;

        part3 - part4
    }

    // 计算二阶导数
    pub fn second_derivative(&self, k: f64, b: f64) -> f64 {
        let part1 = self.rtt_min as f64 * self.t / self.rtt_now as f64;
        let part2 = k.powf(self.n as f64);
        let part3 = k.ln();
        let part4 = part1 * (-part3 * (2.0 - self.n as f64 * part3)) / part2;

        part4
    }
}
enum OptimizationAlg {
    GradientDecent,
    Newton,
}
struct OptimizationStats {
    n_minus_one: Option<OptimizationParas>,
    n_plus_one: Option<OptimizationParas>,
    n: f64,
    k: f64,
    b: f64,
    len: f64,
    opt_alg: OptimizationAlg,
...
impl OptimizationStats {
    pub fn new() -> Self {
        ...
        // 生成文件名
        let filename = format!("traffic_data_{}.csv", now_time);
        Self {
            n_minus_one: None,
            n_plus_one: None,
            n,
    ...
    pub fn start(&mut self, conn: &mut Connection) -> Result<()> {
        if ((self.last_time.elapsed().as_micros() as u64) < conn.get_active_path_mut().unwrap().stats().srtt * 6 ) || (self.last_time.elapsed().as_micros() as u64) < 50000 {
            return Ok(());
        }
        ...
        // }
        match &self.opt_alg {
            OptimizationAlg::GradientDecent => {
                if self.n_minus_one.is_none() {
                    self.n_minus_one = Some(OptimizationParas::new(self.n.round() as i64));
                    self.n_minus_one.as_mut().unwrap().update(conn, &mut self.connection_stats, &mut self.last_time, &mut self.start_time, self.k, self.b, true, self.filename.clone()).unwrap();
                    self.n += 2.0;
                } else {
                    self.n_plus_one = Some(OptimizationParas::new(self.n.round() as i64));
                    let n_plus_one = self.n_plus_one.as_mut().unwrap();
                    n_plus_one.update(conn, &mut self.connection_stats, &mut self.last_time, &mut self.start_time, self.k, self.b, true, self.filename.clone()).unwrap();
                    n_plus_one.calculate_u_derivative(self.n_minus_one.as_mut().unwrap(), self.k, self.b).unwrap();

                    if (self.len > 0 as f64 && self.n_plus_one.as_mut().unwrap().gradient > 0 as f64) ||
                        (self.len < 0 as f64 && self.n_plus_one.as_mut().unwrap().gradient < 0 as f64) {
                        self.learning_rate = self.learning_rate + 1.0;
                        // if self.learning_rate > 256 { self.learning_rate = 256 };
                    } else {
                    ...
                    self.len = self.learning_rate as f64 * (self.n_plus_one.as_mut().unwrap().gradient / self.n_minus_one.as_mut().unwrap().u);
                    // if self.len < 0.2 && self.len > -1.0 {
                    //     self.len = - 1.0
                    // }
                    self.n += self.len.round() - 2.0;
                    // self.n += 2.0;
                    // if (self.len < 0.0) {
                    //     self.n = self.n / 2.0;
                    // }
                    println!("n = {},learning_rate = {},len ={}", self.n, self.learning_rate, self.len);
                    if self.n < 1.0 {
                        self.n = 1.0;
                    }
                    if self.n + 2.0 > (2 as f64 / self.k.ln()) {
                        self.n = (2 as f64 / self.k.ln()) - 2.0;
                    }

                    self.n_plus_one = None;
                    self.n_minus_one = None;
                }
            }
            OptimizationAlg::Newton => {
                self.n_plus_one = Some(OptimizationParas::new(self.n.round() as i64));
                let n_plus_one = self.n_plus_one.as_mut().unwrap();
                n_plus_one.update(conn, &mut self.connection_stats, &mut self.last_time, &mut self.start_time, self.k, self.b, true, self.filename.clone()).unwrap();
                let f1 = n_plus_one.first_derivative(self.k, self.b);
                let f2 = n_plus_one.second_derivative(self.k, self.b);
                println!("n = {},f1 = {},f2 ={}", self.n, f1, f2);
                self.n = (self.n - 0.03 * (f1 / f2)).round();
                if self.n > (2 as f64 / self.k.ln()) {
                    self.n = (2 as f64 / self.k.ln()) - 1.0;
                }
                if self.n < 1.0 {
                    self.n = 1.0;
                }
        ...
        // self.n_plus_one.update(conn, &mut self.connection_stats, &mut self.last_time).unwrap();
        // if self.n_minus_one.is_some() {
        //     self.n_plus_one.as_mut().unwrap().calculate_u(self.k, self.b).unwrap();
        //     self.n_plus_one.as_mut().unwrap().calculate_u_derivative(self.n_minus_one.as_ref().unwrap(), self.k, self.b).unwrap();
        //
        //
        //     if self.n_plus_one.gradient > 0 as f64 && self.n_minus_one.as_mut().unwrap().gradient > 0 as f64
        //         || self.n_plus_one.gradient < 0 as f64 && self.n_minus_one.as_mut().unwrap().gradient < 0 as f64
        //     {
        ...
        //     }
        //
        //     self.n_minus_one = Some(self.n_plus_one.clone());
        //
        //     let len = self.learning_rate as f64 * (self.n_plus_one.gradient / self.n_minus_one.as_ref().unwrap().u);
        //     println!("n = {},learning_rate = {}, gradient = {},len ={}", self.n_plus_one.n, self.learning_rate, self.n_plus_one.gradient, len);
        //
        //     if len < 0 as f64 {
        //         if self.n_plus_one.n + len.floor() as i64 <= 0 {
        //             if self.n_plus_one.n == 1 {
        //                 self.n_minus_one = None;
        //                 print!("reset1");
        //             }
        //             self.n_plus_one.n = 1;
        //         } else {
        //             self.n_plus_one.n += len.floor() as i64;
        //         }
        //     } else {
        //         self.n_plus_one.n += len.ceil() as i64;
        //     }
        //     if self.n_plus_one.n >= (2 as f64 / self.k.ln()).ceil() as i64 {
        //         self.n_plus_one.n = (2 as f64 / self.k.ln()).floor() as i64;
        //     }
        // } else {
        //     self.n_plus_one.calculate_u(self.k, self.b).unwrap();
        //     self.n_minus_one = Some(self.n_plus_one.clone());
        //     self.n_plus_one.n = self.n_plus_one.n + 1;
        // }
...
struct Worker {
    ...
    optimization_stats: Rc<RefCell<OptimizationStats>>,
}
...
impl Worker {
    ...
    pub fn new(
        ...
        let optimization_stats = Rc::new(RefCell::new(OptimizationStats::new()));
    ...
    fn create_new_conns(&mut self) -> Result<()> {
        ...
        while worker_ctx.concurrent_conns < self.option.max_concurrent_conns {
            match self.endpoint.connect(
                ...
                Ok(_) => {
                    println!("{}", worker_ctx.concurrent_conns);
    ...
    fn try_send_requests(&mut self) {
        ...
        for (index, sender) in senders.iter_mut() {
            let conn = self.endpoint.conn_get_mut(*index).unwrap();
            self.optimization_stats.borrow_mut().start(conn).unwrap();
            let n = self.optimization_stats.borrow().n;
            sender.send_requests(conn, n.round() as i64);
            sender.shutdown_streams_ton(n.round() as i64, conn);
            return;
        }
    }
...
impl RequestSender {
    ...
    /// Send requests.
    pub fn send_requests(&mut self, conn: &mut Connection, n: i64) {
        debug!(
            "{} send requests {} {} {} {}",
            conn.trace_id(),
        ...
        while self.concurrent_requests < self.option.max_concurrent_requests
            ...
            || self.request_sent < self.option.max_requests_per_conn) && self.concurrent_requests < n as u64
        {
    ...
    pub fn shutdown_streams_ton(&mut self, n: i64, conn: &mut Connection) {
        let num = self.concurrent_requests as i64 - n;
        if num > 0 {
            let iter = self.streams.iter_mut().take(num as usize);
            // println!("iter_len = {}", iter.len());

            let mut ids_to_remove = Vec::new();

            for (stream_id, _) in iter {
                // println!("stream_id = {stream_id}");
                conn.stream_shutdown(*stream_id, Read, 0).unwrap();
                self.request_done += 1;
                self.concurrent_requests -= 1;
                self.worker_ctx.borrow_mut().request_done += 1;

                ids_to_remove.push(*stream_id);
            }

            // 在迭代结束后，统一删除需要移除的 stream_id
...
impl WorkerHandler {
    ...
    fn try_send_request(&mut self, conn: &mut Connection) {
        ...
        if let Some(s) = sender {
            // self.optimization_stats.borrow_mut().start(conn).unwrap();
            let n = self.optimization_stats.borrow().n;
            s.send_requests(conn, n.round() as i64);
            // s.send_requests(conn, 1);
            // s.shutdown_streams_ton(n, conn);
            return;
        } else {
